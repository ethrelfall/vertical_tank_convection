# Leeds_vertical_convection_MIT.py
...
# this example reproduced Nusselt number from the benchmark at
# https://wwwold.mathematik.tu-dortmund.de/~featflow/en/benchmarks/cfdbenchmarking/mit_benchmark/mit_result.html
...
amp_0 = 0.71  # Prandtl number for air (room temp)
...
# solve
for i in range (0,8):  # continuation optional, no continuation means run once (use range (0,1))

   # this is to get continuation to Pr = 0.71, Ra = 3.4e5, height=8, as in MIT benchmark
   Ra = pow(10,3+0.5*i)
   if i==4:
      Ra=5.0e4
   if i==5:
...
File("Leeds_vertical_convection_MIT.pvd").write(u,p,T,Pr)
...
File("Leeds_vertical_convection_MIT_norm.pvd").write(T_n,Pr_n)
