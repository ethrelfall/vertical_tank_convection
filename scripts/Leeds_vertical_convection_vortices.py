...
# this example generates a steady-state flow with vortex cells
# it was done by examining the numerical behaviour of low-Pr fluids, not taken from literature
...
# solve
for i in range (0,15):  # continuation optional, no continuation means run once (use range (0,1))
...
File("Leeds_vertical_convection_vortices.pvd").write(u,p,T,Pr)
...
File("Leeds_vertical_convection_vortices_norm.pvd").write(T_n,Pr_n)
