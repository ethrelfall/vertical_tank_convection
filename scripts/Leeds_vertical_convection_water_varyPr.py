...
# this example enables examination of T-dependent Pr for water: is set so Pr = 7.0 on cold side, 2.0 on hot side
...
amp_coeffs[1] = -5.0  # for water try -5.0 for initial attempt, means Pr is 2.0 at hot edge and 7.0 at cold edge (room temp)
...
File("Leeds_vertical_convection_water_varyPr.pvd").write(u,p,T,Pr)
...
File("Leeds_vertical_convection_water_varyPr_norm.pvd").write(T_n,Pr_n)
